    ...
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 bg-bg-dark/70 backdrop-blur-xl border-b border-glass-border">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          <div className={`w-2 h-2 rounded-full ${isConfigValid() ? 'bg-secondary animate-pulse' : 'bg-accent'}`} />
          <span className="text-xs text-white/50">
            {isConfigValid() ? 'API Ready' : 'API Not Configured'}
          </span>
        </div>
        ...
        <ul className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <li key={link.path}>
              <Link
                to={link.path}
                className={`relative text-sm font-medium transition-colors flex items-center gap-2 ${
                  location.pathname === link.path
                    ? 'text-white'
                    : 'text-white/70 hover:text-white'
                }`}
              >
                {link.path === '/configure' && <Settings className="w-4 h-4" />}
                {link.label}
                {location.pathname === link.path && (
